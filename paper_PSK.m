...
long=16;
close all;
PSK_mod=comm.PSKModulator('ModulationOrder',8);
        %PSK_Demod=comm.PSKDemodulator('ModulationOrder',M);
        %PSK_mod.PhaseOffset = phase_offset;
        s_PSK=step(PSK_mod,repmat((0:7)',2000/long,1));
        signal_last=s_PSK;
        %fo=randn(length(signal_last),1)/50000;
        fo=normrnd(0,0.03,length(signal_last),1);
...
label_PSK=repmat((1:8)',2000/long,1);
...
%         s_QAM=step(QAM_mod,(0:7)');
        PSK_mod=comm.PSKModulator('ModulationOrder',8);
        %PSK_mod.PhaseOffset = phase_offset;
        s_PSK=step(PSK_mod,(0:7)');
h10=scatter(real(s_PSK),imag(s_PSK),'y.');
hold on;
axis equal;
text(real(s_PSK(1)),imag(s_PSK(1)),'0');
text(real(s_PSK(2)),imag(s_PSK(2)),'1');
text(real(s_PSK(3)),imag(s_PSK(3)),'2');
text(real(s_PSK(4)),imag(s_PSK(4)),'3');
text(real(s_PSK(5)),imag(s_PSK(5)),'4');
text(real(s_PSK(6)),imag(s_PSK(6)),'5');
text(real(s_PSK(7)),imag(s_PSK(7)),'6');
text(real(s_PSK(8)),imag(s_PSK(8)),'7');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% order_kmeans=[6,7,8,4,3,5,1,2];
% order_ng=[2,6,1,4,5,3,8,8];
% order_my=[3,2,8,1,5,4,7,6];
