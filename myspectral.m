function [IDX,center,eigenvalue]=myspectral(data,k,weight)
sigma_radius=0.7;
sigma_ang=0.1;
affinity = CalculateAffinity(data);
width=size(data,2);
radius=sqrt((data.*conj(data))*ones(width,1));
radius_matrix=repmat(radius,1,length(radius));
radius_diff=abs(radius_matrix-transpose(radius_matrix));
%ang=angle(data)*ones(width,1);
ang=angle(data*[1;1i]);
angle_matrix=repmat(ang,1,length(ang));
angle_diff=abs(angle_matrix-transpose(angle_matrix));
affinity=affinity.*weight.*exp(-radius_diff.^2/2/sigma_radius^2);%.*exp(-angle_diff.^2/2/sigma_ang^2);
affinity=affinity/max(max(affinity));
% figure;
% %[XX,YY]=meshgrid(data(:,1),data(:,2));
% scatter3(data(:,1),data(:,2),density.*exp(-radius_diff.^2/2/sigma_radius^2).*exp(-angle_diff.^2/2/sigma_ang^2));
% grid on;
